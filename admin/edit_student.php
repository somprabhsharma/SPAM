...
<title>Admin | Edit Rent Details</title>
 <script>
				function onupdate()
					{
...
<p><?php
	@$id = $_GET['edit_hide'];
	$query = "select * from bsr_bsr where id='$id'";
...
<?php
	...
	{
		$query1="update bsr_bsr set type='$update_type',propertytype='$update_ptype',city='$update_city',propertyname='$update_pname',address='$update_add',description='$update_des',contact='$update_cont',price='$update_price',keyword='$update_key',image='$update_image' where id='$id2'";
		$sql1=mysql_query($query1);
		
		if($sql1)
			{
				echo '<script> alert("You Have Successfully Updated Your Details!!")</script>';								
				echo "<script>window.location='view_rent.php'</script>";
			}
		else
			{
